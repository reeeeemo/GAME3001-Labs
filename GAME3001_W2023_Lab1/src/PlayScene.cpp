...
{
	if (Util::Distance(glm::vec2(m_pPlayer->GetTransform()->position.x, 0.0f), glm::vec2(800.0f, 0.0f)) <= 1.0f) { // If player is on the very right side of the screen
		Game::Instance().ChangeSceneState(SceneState::END); // Change to END scene
	}
	UpdateDisplayList();
}
...
void PlayScene::HandleEvents()
{
	...
	if (SDL_NumJoysticks() < 1)
	{
		...
		{
			m_pPlayer->GetTransform()->position -= glm::vec2(0.0f,1.0f);
		...
		if (EventManager::Instance().IsKeyDown(SDL_SCANCODE_A))
		{
			...
			m_pPlayer->GetTransform()->position -= glm::vec2(1.0f, 0.0f);
			m_playerFacingRight = false;
		}
